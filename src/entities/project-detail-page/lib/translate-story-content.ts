...
import {
  ...
  isI18nCodeBlock,
  isI18nTextBlock,
} from '@/entities/project-detail-page/lib/block-utils';
import { STORY_MEDIA_TEXT_BLOCK_TYPE } from '@/entities/project-detail-page/lib/story-layout-types';
import {
  protectNonTranslatableHtml,
  restoreNonTranslatableHtml,
} from '@/entities/project-detail-page/lib/protect-translatable-html';
import {
  mapEditorBlocksWithPaths,
  walkEditorBlocksWithPaths,
} from '@/entities/project-detail-page/lib/story-blocks';
...
function isTranslatableHtmlBlock(type: string): boolean {
  return isI18nTextBlock(type) || type === STORY_MEDIA_TEXT_BLOCK_TYPE;
}
...
export async function translateStoryContent(
  ...
  walkEditorBlocksWithPaths(content.blocks, (block, path) => {
    ...
    if (!isTranslatableHtmlBlock(block.type)) return;
