...
import { createEditorMediaTextTool } from './editor-media-text-tool';
import { createEditorMediaRowTool } from './editor-media-row-tool';
...
export function createEditorTools(
  ...
  return {
    ...
    mediaText: {
      class: createEditorMediaTextTool({
        uploadImage: uploadByFile,
        uploadVideo:
          uploadVideo ?? (async (file) => uploadByFile(file)),
      }) as unknown as ToolConstructable,
    },
    mediaRow: {
      class: createEditorMediaRowTool({
        uploadImage: uploadByFile,
        uploadVideo:
          uploadVideo ?? (async (file) => uploadByFile(file)),
      }) as unknown as ToolConstructable,
