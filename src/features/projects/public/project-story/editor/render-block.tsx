...
function renderMediaText(
  ...
  const data = readStoryMediaTextData(block.data);
  const html = getBlockText(block, locale) || data.html || '';
...
function renderMediaRow(data: Record<string, unknown>): ReactNode {
  const row = readStoryMediaRowData(data);

  return (
    <StoryMediaRowBlock
      left={{
        kind: row.left.kind,
        url: row.left.file?.url,
        caption: row.left.caption,
      }}
      right={{
        kind: row.right.kind,
        url: row.right.file?.url,
...
export function renderBlock(block: EditorBlock, locale: I18nLocale): ReactNode {
  switch (block.type) {
    ...
    case 'mediaText':
      return renderMediaText(block, locale);
    case 'mediaRow':
      return renderMediaRow(block.data);
    case 'embed':
      return (
        <EmbedBlock
